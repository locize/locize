...
export function initUI(options) {
  ...
  let style =
    'font-family: "Helvetica", "Arial", sans-serif; bottom: 20px; right: 20px; padding: 10px; background-color: #fff; border: solid 1px #1976d2; box-shadow: 0px 1px 2px 0px rgba(0,0,0,0.5); border-radius: 3px;';
  ...
  turnOn.onclick = () => {
    ...
    const backendOptions =
      i18next && i18next.options && i18next.options.backend;
    const { projectId, version } = { ...backendOptions, ...options };

    window.location = `https://www.locize.app/cat/${projectId}/v/${version}/incontext?sourceurl=${encodeURI(
